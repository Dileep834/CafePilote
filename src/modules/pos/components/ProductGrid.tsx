...
export function ProductGrid() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) => {
      const matchesCategory =
        selectedCategory === 'All' || (p.categories as any)?.name === selectedCategory;
  ...
  return (
    <div className="flex flex-col h-full gap-2.5 sm:gap-5 bg-white sm:bg-transparent rounded-t-2xl sm:rounded-none px-1 sm:px-0 pt-2 sm:pt-0 pb-28 sm:pb-0 -mx-1 sm:mx-0 min-h-0">
      {/* Categories — compact chips on mobile (no tall square cards) */}
      ...
      <div className="flex items-center gap-2 sm:gap-4 px-0.5 sm:px-0">
        ...
        <div className="hidden sm:flex items-center gap-3 ml-auto flex-1 justify-end max-w-2xl">
          ...
          <Button
            className="h-10 px-4 bg-brand-orange hover:bg-[#e55f00] text-white rounded-xl shadow-md"
            onClick={clearCart}
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain px-0.5 sm:px-0 pb-4 md:pb-2">
        {filteredProducts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center text-slate-400">
            ...
            <p className="text-sm font-semibold text-slate-500">No products match</p>
            <p className="text-xs mt-1">Try another category or clear search</p>
          </div>
        ) : (
            ...
            {filteredProducts.map((product) => (
              ...
              >
                <div className="flex items-start justify-between z-10 w-full mb-1.5">
                  ...
                  <button
                    type="button"
                    className="text-slate-300 p-1 -mr-1 -mt-1"
