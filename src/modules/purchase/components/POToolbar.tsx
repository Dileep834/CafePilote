...
import type { POFilters, POStatusFilter } from '../lib/poHelpers';
...
type Props = {
  ...
  onChange: (next: Partial<POFilters>) => void;
  onReset: () => void;
  onExport: () => void;
};
  ...
  return (
    <div className="sticky top-0 z-20 -mx-1 space-y-2 rounded-b-xl rounded-t-none border-b border-slate-100 bg-white p-3 shadow-[0_4px_12px_rgba(15,23,42,0.06)] sm:mx-0 sm:space-y-3 sm:rounded-xl sm:border-0 sm:p-4 sm:shadow-sm sm:ring-1 sm:ring-slate-100">
      ...
      <div className={cn('grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3 xl:flex xl:flex-wrap', filtersOpen ? 'grid' : 'hidden lg:grid xl:flex')}>
        <select className={cn(selectClass, 'w-full xl:min-w-[140px] xl:w-auto')} value={filters.supplier} onChange={(e) => onChange({ supplier: e.target.value })} aria-label="Supplier">
        ...
        <select
          ...
          onChange={(e) => onChange({ status: e.target.value as POStatusFilter })}
          aria-label="Status"
        >
        ...
        <select className={cn(selectClass, 'w-full xl:min-w-[140px] xl:w-auto')} value={filters.outlet} onChange={(e) => onChange({ outlet: e.target.value })} aria-label="Outlet">
          <option value="all">Outlet</option>
          {outlets.map((o) => (
            <option key={o.id} value={o.id}>{o.name}</option>
          ))}
        </select>
        <input type="date" className={cn(selectClass, 'w-full xl:w-auto')} value={filters.expectedDate} onChange={(e) => onChange({ expectedDate: e.target.value })} aria-label="Expected delivery" />
