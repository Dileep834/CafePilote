...
import type { AdjustmentRow, AdjustmentStatus } from './types';

type Props = {
  rows: AdjustmentRow[];
  canApprove: boolean;
  canDelete: boolean;
  onView: (row: AdjustmentRow) => void;
  onApprove: (row: AdjustmentRow) => void;
  onReject: (row: AdjustmentRow) => void;
  onDelete: (row: AdjustmentRow) => void;
};

const STATUS_STYLE: Record<AdjustmentStatus, string> = {
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-slate-100 text-sm hover:bg-slate-50/80">
                ...
                <td className="px-3 py-3 text-right tabular-nums text-slate-600">{row.previousStock}</td>
                <td
                  className={cn(
                    'px-3 py-3 text-right tabular-nums font-bold',
                    row.adjustment >= 0 ? 'text-emerald-600' : 'text-red-600'
                  )}
                >
                  {row.adjustment > 0 ? `+${row.adjustment}` : row.adjustment}
                </td>
                <td className="px-3 py-3 text-right tabular-nums font-semibold text-slate-900">{row.newStock}</td>
                <td className="px-3 py-3 text-slate-700">{row.reason}</td>
                <td className="px-3 py-3">
                  <span className={cn('inline-flex rounded-full px-2.5 py-1 text-[12px] font-semibold capitalize', STATUS_STYLE[row.status])}>
                    {row.status}
                  </span>
                </td>
                <td className="px-3 py-3 text-slate-600">{row.employee || '—'}</td>
                <td className="px-3 py-3 text-slate-600">{row.approvedBy || '—'}</td>
                <td className="px-3 py-3 text-right">
                  <DropdownMenu>
                    <DropdownMenuTrigger
                      render={
                        <Button type="button" variant="ghost" size="icon" className="h-9 w-9 rounded-xl" aria-label={`Actions for ${row.productName}`}>
