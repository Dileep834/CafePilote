...
import { WASTE_REASONS, type WasteFilters, type WasteStatus } from './types';
...
type Props = {
  ...
  onChange: (next: Partial<WasteFilters>) => void;
  onReset: () => void;
  onExport: () => void;
};

const STATUS_OPTIONS: { value: 'all' | WasteStatus; label: string }[] = [
  ...
  return (
    <div className="sticky top-0 z-20 -mx-1 space-y-2 rounded-b-xl rounded-t-none border-b border-slate-100 bg-white p-3 shadow-[0_4px_12px_rgba(15,23,42,0.06)] sm:mx-0 sm:space-y-3 sm:rounded-xl sm:border-0 sm:p-4 sm:shadow-sm sm:ring-1 sm:ring-slate-100">
      ...
      >
        ...
        <input
          type="date"
          className={cn(selectClass, 'w-full xl:w-auto')}
        ...
        <select
          ...
          onChange={(e) => onChange({ status: e.target.value as WasteFilters['status'] })}
